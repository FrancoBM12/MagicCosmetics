...
public class WStickMenu extends PaginatedMenu {
    ...
    public WStickMenu(String id, ContentMenu contentMenu, int startSlot, int endSlot, int backSlot, int nextSlot, int pagesSlot, List<Integer> slotsUnavailable) {
    ...
    public WStickMenu(PlayerCache playerCache, Menu menu) {
    ...
    public void handleMenu(InventoryClickEvent event) {
        ...
        if(slotMenu.getSlot() == getNextSlot()){
            slotMenu.playSound(player);
            List<Cosmetic> cosmetics = Cosmetic.getCosmeticsByType(CosmeticType.WALKING_STICK);
    ...
    public void setItems() {
        getContentMenu().removeSlotMenu(getPagesSlot());
        if(getBackSlot() != -1) {
        ...
        if(!cosmetics.isEmpty()) {
            for (int i = 0; i < getMaxItemsPerPage(); i++) {
                ...
                if(playerCache.getWStick() != null){
                    if(playerCache.getWStick().getId().equalsIgnoreCase(cosmetic.getId())){
