...
public class PlayerBagHandler extends PlayerBag {
    ...
    public void spawnSelf(Player player) {
        ...
        if(height > 0){
            for(int i = 0; i < height; i++) {
                ...
                ids.add(entityAreaEffectCloud.aj());
            }
            for(int i = 0; i < height; i++) {
                if(i == 0){
            ...
            addPassenger(player, ids.get(ids.size() - 1), armorStand.aj());
        }else{
            addPassenger(player, lendEntityId == -1 ? owner.getEntityId() : lendEntityId, armorStand.aj());
        }
    ...
    public void addPassenger(boolean exception) {
        List<Packet<?>> backPack = getBackPackMountPacket(lendEntityId == -1 ? getPlayer().getEntityId() : lendEntityId, armorStand.aj());
        for(Player player : getPlayersInRange()){
    ...
    private List<Packet<?>> getBackPackSpawn(ItemStack backpackItem) {
        ...
        PacketPlayOutEntityMetadata entityMetadata = new PacketPlayOutEntityMetadata(armorStand.aj(), armorStand.an().c());
        PacketPlayOutMount mountEntity = new PacketPlayOutMount(entityPlayer);
        PacketPlayOutEntityEquipment equip = new PacketPlayOutEntityEquipment(armorStand.aj(), Collections.singletonList(new Pair<>(EnumItemSlot.f, CraftItemStack.asNMSCopy(backpackItem))));
    ...
    private List<Packet<?>> getBackPackDismount(boolean removeClouds) {
        ...
        if(!removeClouds) {
            PacketPlayOutEntityDestroy backPackDestroy = new PacketPlayOutEntityDestroy(armorStand.aj());
            return Collections.singletonList(backPackDestroy);
        }
        for (Integer id : ids) {
            packets.add(new PacketPlayOutEntityDestroy(id));
        }
        packets.add(new PacketPlayOutEntityDestroy(armorStand.aj()));
        return packets;
    }
    ...
    private List<Packet<?>> getBackPackHelmetPacket(ItemStack itemStack) {
        ...
        return Collections.singletonList(new PacketPlayOutEntityEquipment(armorStand.aj(), list));
    }

    private List<Packet<?>> getBackPackHelmetPacket(ArrayList<Pair<EnumItemSlot, net.minecraft.world.item.ItemStack>> pairs) {
        return Collections.singletonList(new PacketPlayOutEntityEquipment(armorStand.aj(), pairs));
    }
    ...
    private List<Packet<?>> getBackPackRotationPackets(float yaw) {
        ...
        PacketPlayOutEntity.PacketPlayOutEntityLook packetPlayOutEntityLook = new PacketPlayOutEntity.PacketPlayOutEntityLook(armorStand.aj(), (byte) (yaw * 256 / 360), /*(byte) (pitch * 256 / 360)*/(byte)0, true);
