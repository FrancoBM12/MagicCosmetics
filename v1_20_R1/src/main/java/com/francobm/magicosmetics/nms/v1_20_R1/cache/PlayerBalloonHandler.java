...
public class PlayerBalloonHandler extends PlayerBalloon {
    ...
    public void spawn(Player player) {
        ...
        //client settings
        entityPlayer.c.a(new PacketPlayOutSpawnEntity(leashed));
        leashed.aj().refresh(entityPlayer);
        if(!invisibleLeash) {
            entityPlayer.c.a(new PacketPlayOutAttachEntity(leashed, lendEntity == null ? ((CraftPlayer) owner).getHandle() : ((CraftLivingEntity)lendEntity).getHandle()));
        }
        //client settings
    ...
    protected void instantUpdate() { //implement this method to others versions
        ...
        for(UUID uuid : viewers){
            ...
            if(!invisibleLeash) {
                p.c.a(new PacketPlayOutAttachEntity(leashed, lendEntity == null ? ((CraftPlayer) owner).getHandle() : ((CraftLivingEntity)lendEntity).getHandle()));
            }
            armorStand.aj().refresh(p);
            p.c.a(new PacketPlayOutEntityTeleport(leashed));
            p.c.a(new PacketPlayOutEntityTeleport(armorStand));
        }
    }
